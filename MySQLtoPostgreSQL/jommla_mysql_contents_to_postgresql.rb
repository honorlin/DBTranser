...
ActiveRecord::Base.transaction do
  Qn6u7Content.find_each(batch_size: 5000) do |c|
    article = Article.new
    article.id = c.id
    article.title = c.title
    #article.summary =   
    article.content = c.introtext
    article.categories_text = Category.find_by(id: c.catid).title
    article.created_at = c.created
    article.created_by = c.created_by
    article.modified_by = c.modified_by
    article.start_date = c.publish_up
    article.end_date = c.publish_down
    article.images = c.images
    article.hits = c.hits
    article.state_id = c.state
